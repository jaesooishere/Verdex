...
import { generateReportId, storeReport } from "@/lib/report-storage"
...
export async function POST(req: Request) {
  try {
    ...
    // Use gemini-2.5-flash-lite which is optimized for free tier
...
No periods, no spaces, no explanation, no other characters.`
    ...
    // Check if first character is 'y' (in case model adds extra chars)
    ...
    const reportPrompt = `You are Verdex, an elite business analysis AI that provides consultant-grade strategic reports. You emulate the judgment of a senior McKinsey consultant with decades of experience.
...
IMPORTANT: Return ONLY the JSON object. No markdown formatting, no \`\`\`json, no explanation before or after.`
    ...
    // Return success status and encoded report data
    return Response.json({
      isValid: true,
      reportData: encodedReport,
      reportReady: true,
    })
  } catch (error) {
